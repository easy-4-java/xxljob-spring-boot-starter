...
public class XxlJobAutoBindingSpringExecutor extends XxlJobSpringExecutor implements DisposableBean {
    ...
    private void startRetryScheduler() {
        ...
        retryScheduler.scheduleWithFixedDelay(() -> {
            ...
            List<XxlJobInfo> snapshot = new ArrayList<>(failedJobs);
            for (XxlJobInfo job : snapshot) {
                try {
                    // 自愈只查询+添加，不更新（避免覆盖管理界面的手动修改/停止）
                    ReturnT<XxlJobInfoList> queryResult = getXxlJobTemplate().jobInfoList(0, Integer.MAX_VALUE, job.getJobGroup());
                    if (queryResult.getCode() == ReturnT.SUCCESS_CODE && queryResult.getContent() != null
                            && queryResult.getContent().getData() != null) {
                        boolean exists = queryResult.getContent().getData().stream()
                                .anyMatch(j -> j.getExecutorHandler().equals(job.getExecutorHandler()));
                        if (exists) {
                            log.info(">>>>>>>>>>> xxl-job self-healing: handler={} already exists in admin, skip", job.getExecutorHandler());
                            failedJobs.remove(job);
                            continue;
                        }
                    }
                    // 任务不存在，执行添加
                    ReturnT<String> result = getXxlJobTemplate().addJob(job);
                    if (result.getCode() == ReturnT.SUCCESS_CODE) {
                        log.info(">>>>>>>>>>> xxl-job self-healing SUCCESS: handler={} added", job.getExecutorHandler());
                        failedJobs.remove(job);
                    } else {
                        log.warn(">>>>>>>>>>> xxl-job self-healing retry: handler={} add failed: {}", job.getExecutorHandler(), result.getMsg());
                    }
                } catch (Exception e) {
                    log.error(">>>>>>>>>>> xxl-job self-healing retry error: handler={}", job.getExecutorHandler(), e);
                }
            }
